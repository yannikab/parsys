...
#pragma omp parallel num_threads(2) private (i,j,c)
		{
			unsigned int n;

			for (n = 0; n < ITERATIONS; n++)
			{
#pragma omp master // master handles MPI messaging
				{
				...
				/* Wait for recvs. Master handles MPI messaging. */
#pragma omp master
				MPI_Waitall(q, recvs, recv_status);

				/* Handle diagonal border data that require recvs to have been completed, master can safely do it. */
#pragma omp master
				{
				...
				/* Apply outer filter, requires having border data available, master can safely do it. */
#pragma omp master
				apply_outer_filter(prev_image, curr_image, B + height + B, B + width + B);

				/* Wait for sends before we switch buffers. Master handles MPI messaging. */
#pragma omp master
				MPI_Waitall(p, sends, send_status);

				/* Switch current / previous image buffers. Master does it after sends have completed. */
#pragma omp master
				{
					float (**tmp)[CHANNELS];
					tmp = prev_image;
				...
				/* Threads can't move to next iteration until master has finished switching buffers. */
#pragma omp barrier
			}
		}
