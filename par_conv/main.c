...
int main(int argc, char** argv)
{
	...
	//	if (rank == 0)
	//	{
	unsigned char (**input_buffer)[CHANNELS]; // one buffer of arrays of 3 bytes

	/* Read input image. */

	if (read_image((unsigned char ***) &input_buffer))
		printf("OK\n");

	//	alloc_uchar_array((unsigned char ***) &input_buffer, HEIGHT, WIDTH, CHANNELS);

	unsigned char **output_buffer[CHANNELS];

	for (c = 0; c < CHANNELS; c++)
		alloc_uchar_array(&(output_buffer[c]), HEIGHT, WIDTH, 1);

	for (c = 0; c < CHANNELS; c++)
		for (i = 0; i < HEIGHT; i++)
			for (j = 0; j < WIDTH; j++)
				output_buffer[c][i][j] = input_buffer[i][j][c];
	...
	for (c = 0; c < CHANNELS; c++)
		for (i = 0; i < HEIGHT; i++)
			fwrite(output_buffer[c][i], 1, WIDTH, out_fp[c]);
	...
	//			int x = (i - 1) % s;
	//			int y = (i - 1) / s;
	//			MPI_Send(&(input_buffer[y * local_height][x * local_width]), 1, local_image_t, i, 0, MPI_COMM_WORLD);
	//			//			int d;
	...
	//		printf("Slave: %d \n", rank);
	//		MPI_Status status;
	//		MPI_Recv(local_buffer, local_height * local_width * CHANNELS, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD, &status);
	//		//		int d;
	...
	//		unsigned char (**output_channel)[local_width];
	//
	//		for (c = 0; c < CHANNELS; c++)
	//		{
	//			output_channel[c] = NULL;
	//			output_channel[c] = malloc(local_height * sizeof (*output_channel[c]));
	...
	//				for (c = 0; c < CHANNELS; c++)
	//					output_channel[c][i][j] = (char) local_buffer[i][j][c];
	//
	//		open_output_files(rank);
	//
	//		/* Write out channels to separate files. */
	//
	//		for (c = 0; c < CHANNELS; c++)
	//			for (i = 0; i < local_height; i++)
	//				fwrite(output_channel[c][i], 1, local_width, out_fp[c]);
	//
	//		close_output_files();
	//
